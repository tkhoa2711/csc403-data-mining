    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < dataTest.length; i++) {
            ...
            for (int j = 0; j < dataTrain.length; j++) {
                distance[j] = 1/getWeightedCosDistance(dataTrain[j], dataTest[i], weight);
    ...
    public static boolean correctlyClassified(Point a, Point[] neighbours, double[] weight, double percentage) {
        ...
        for (int i = 0; i < neighbours.length; i++) {
            if (neighbours[i].getLabel() == label) {
                classSimilarity += 1/getWeightedCosDistance(a, neighbours[i], weight);
            } else {
                otherSimilarity += 1/getWeightedCosDistance(a, neighbours[i], weight);
            }
        }
    ...
    public static int objectiveFunction(Point[] allpoints, double[] weight, double percentage, int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n && j != i; j++) {
                distances[count] = 1/getWeightedCosDistance(allpoints[i], allpoints[j], weight);
                count++;
            }
